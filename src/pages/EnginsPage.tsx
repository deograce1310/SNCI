...
export default function EnginsPage() {
  ...
  const generalWhatsappHref = `https://wa.me/${company.whatsapp}?text=${encodeURIComponent(
    en ? 'Hello SNCI, I would like information about the equipment you offer for rental and sale.' : 'Bonjour SNCI, je souhaite des informations sur les engins que vous proposez à la location et à la vente.'
  )}`;
      ...
      <section className="bg-[#F6F2F2] pt-32 lg:pt-40 pb-16 lg:pb-20">
        <div className="max-w-[1200px] mx-auto px-5 lg:px-10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="font-mono text-xs font-medium uppercase tracking-[0.08em] text-[#CF0D0D]">{en ? 'EQUIPMENT RENTAL & SALE' : 'LOCATION ET VENTE D\'ENGINS'}</span>
          </div>
          <h1 className="font-archivo font-bold text-[clamp(32px,4vw,48px)] text-[#0A090E]">{en ? 'Equipment rental & sale' : 'Location et vente d\'engins'}</h1>
          <p className="text-lg text-[#475479] mt-4 max-w-2xl">
            {en
              ...
              : 'SNCI propose à la location et à la vente une gamme d\'engins pour l\'accès en hauteur et le terrassement. Ouvrez une fiche, puis contactez-nous sur WhatsApp pour le prix, la fiche technique et la disponibilité.'}
          </p>
        </div>
      </section>
